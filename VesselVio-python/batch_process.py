...
import numpy as np
from multicore import TaskRunner
...
def process_volume(
    volume_file, skeleton_dir, gen_options, ann_options, vis_options, iteration, verbose
):
    ...
    resolution = ImProc.prep_resolution(gen_options.resolution) # list -> np.array
    ...
    for i, roi_name in enumerate(annotation_data.keys()):
        ...
        volume, image_shape = ImProc.load_npz(volume_file, verbose=verbose)
        ...
        points = VolProc.skeletonize(volume, verbose=verbose) # (n,3)
        ...
        v_coords = VolProc.absolute_points(points, point_minima)  # Z,X,Y
        #print(point_minima)

        z_max, x_max, y_max = skeletonized_volume.shape
        out_of_bounds = v_coords[
            (v_coords[:,0] >= z_max) |
            (v_coords[:,1] >= x_max) |
            (v_coords[:,2] >= y_max)
        ]
        #print("越界坐标：", out_of_bounds)

        # 保留所有在范围内的点
        mask = (
            (v_coords[:,0] >= 0) & (v_coords[:,0] < z_max) &
            (v_coords[:,1] >= 0) & (v_coords[:,1] < x_max) &
            (v_coords[:,2] >= 0) & (v_coords[:,2] < y_max)
        )

        v_coords = v_coords[mask]
        points = points[mask]

        # save skeleton volume
        skeletonized_volume = np.zeros(image_shape).astype(np.uint8)  # Z,X,Y   
        skeletonized_volume[v_coords[:,0],v_coords[:,1],v_coords[:,2]] = 1
        skeletonized_img = nib.Nifti1Image(skeletonized_volume.astype(np.uint8), affine=np.eye(4))
        nib.save(skeletonized_img, os.path.join(skeleton_dir,filename+'.nii.gz'))
...
def create_graph(file, path_mode, skeleton_mode_dir, gen_options, no_annotation, vis_options, verbose):
    print('*' * 50)
    print('BEGIN PROCESS ID:', file)
    file_path = os.path.join(path_mode, file)
    process_volume(file_path, skeleton_mode_dir, gen_options, no_annotation, vis_options, 0, verbose)
...
if __name__ == "__main__":
    ...
    # Volumes
    load_original_volume = True
    load_smoothed_volume = True
    # Movie options
    ...
    prune_length = 1#5  # Prune end point segments shorter than prune_length
    filter_length = 1#10  # Filter isolated segments shorter than filter_length
    ...
    root = 'your_dataset_root_folder'  
    results_folder = 'your_result_output_folder'
    graph_dir = os.path.join(results_folder, 'graph')
    skeleton_dir = os.path.join(results_folder, 'skeleton')
    for mode in ['train','test', 'val']: 
        print('*' * 40)
        print(mode)
        path_mode = os.path.join(root, mode)
        files = sorted(os.listdir(path_mode)) # e.g. ['pulse_00002_0.npz', ...]

        graph_mode_dir = os.path.join(graph_dir, mode)
        os.makedirs(graph_mode_dir, exist_ok=True)
        skeleton_mode_dir = os.path.join(skeleton_dir, mode)
        os.makedirs(skeleton_mode_dir, exist_ok=True)
        ...
        # single thread
        # for file in files:
        #     create_graph(file, path_mode, skeleton_mode_dir, gen_options, no_annotation, vis_options, verbose)

        runner = TaskRunner(create_graph, files, max_workers=1)
        runner.run(path_mode,skeleton_mode_dir, gen_options, no_annotation, vis_options, verbose)
